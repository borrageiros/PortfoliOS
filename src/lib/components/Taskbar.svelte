<script lang="ts">
	...
	let socialLinks: { color: string; url: string; icon: string; viewBox?: string; name: string }[] =
		[];
	...
	onMount(async () => {
		const contact = await loadData<{ rrss: typeof socialLinks }>('contact');
...
<div class="taskbar">
	...
	<div class="system-tray">
		<div
			class="fullscreen-toggle tray-control"
			on:click={toggleFullscreen}
			on:keydown={handleKeydown}
			role="button"
			tabindex="0"
			title={$t('taskbar.toggleFullscreen')}
		>
			<Icon name={isFullscreen ? 'fullscreen-exit' : 'fullscreen-enter'} size="16" />
		</div>
		<div
			class="theme-toggle tray-control"
			on:click={toggleTheme}
			on:keydown={handleKeydown}
			role="button"
			tabindex="0"
			title={$t('taskbar.theme')}
		>
			<Icon name={getThemeIcon($theme)} size="16" />
		</div>
		<div
			class="language-selector tray-control"
