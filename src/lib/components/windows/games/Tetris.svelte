			...
			{#each grid as row, ri (ri)}
				<div class="grid-row">
					{#each row as cell, ci (ci)}
						<div
							class="grid-cell"
							style="background-color: {cell.filled || cell.color ? cell.color : '#111'}"
						></div>
					{/each}
				</div>
			{/each}
			...
			<div class="next-piece">
				{#if nextPiece}
					<div
						...
						style="grid-template-rows: repeat({nextPiece.shape
							.length}, 1fr); grid-template-columns: repeat({nextPiece.shape[0].length}, 1fr);"
					>
						{#each nextPiece.shape as row, ri (ri)}
							{#each row as cell, ci (ci)}
								<div
