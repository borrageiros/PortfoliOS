<script lang="ts">
	...
	function getCellClass(row: number, col: number) {
		...
		const _boxRowStart = Math.floor(row / 3) * 3;
		const _boxColStart = Math.floor(col / 3) * 3;
		const isInSameBox =
			selectedCell &&
	...
	<div class="game-header">
		...
		<div class="game-controls">
			<div class="difficulty-selector">
				...
				<button
					class="difficulty-btn {difficulty === 'expert' ? 'active' : ''}"
			...
			<div class="game-actions">
				...
				<button
					class="action-btn note-btn {showNotesMode ? 'active' : ''}"
					...
					>
						...
						{:else if cellNotes[rowIndex][colIndex]?.length > 0}
							<div class="cell-notes">
								{#each Array(9).fill(0) as _n, i (i)}
									<div class="note-slot">
										{cellNotes[rowIndex][colIndex].includes(i + 1) ? i + 1 : ''}
									</div>
								{/each}
							</div>
						{/if}
