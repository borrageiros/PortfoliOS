...
<div class="browser">
	<div class="browser-toolbar">
		<div class="browser-controls">
			<button
				...
				aria-label={$t('softwares.browser.back')}
			>
			...
			<button
				...
				aria-label={$t('softwares.browser.forward')}
			>
				<svg viewBox="0 0 24 24" width="16" height="16">
					<path
						fill="currentColor"
	...
	{#if showBookmarks}
		<div class="bookmarks-bar">
			{#each bookMarks as bookmark, i (i)}
				<button
				...
				>
					...
					<span
						>{bookmark.name.length > 15 ? bookmark.name.slice(0, 15) + '...' : bookmark.name}</span
					>
				</button>
			{/each}
		</div>
