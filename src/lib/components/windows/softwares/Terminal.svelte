<script lang="ts">
	...
	const commands: CommandsDirectory = {
		help: {
			...
			execute: (): string[] => {
				return Object.entries(commands).map(
					([cmd]) => `${cmd.padEnd(15)} - ${$t(`softwares.terminal.commands.${cmd}`)}`
				);
			}
		},
		clear: {
		...
		profile: {
			...
			execute: (): string[] => {
				...
				return [
					`${$t('profile.name')}: ${information.name}`,
		...
		projects: {
			...
			execute: (): string[] => {
				if (!projects?.relevant) return [];
				return projects.relevant.projects.map(
		...
		skills: {
			...
			execute: (): string[] => {
				if (!skills) return [];
				return [
					`${$t('profile.skills')}: ${skills.skills.map((skill: { name: string }) => skill.name).join(', ')}`
				];
			}
		},
		contact: {
	...
	function executeCommand(command: string): void {
		...
		if (commands[cmd]) {
			try {
				...
				const errorMessage =
					error instanceof Error ? error.message : $t('softwares.terminal.unknownError');
				addToHistory(
					currentDirectory,
					command,
					[`${$t('softwares.terminal.errorExecuting')}: ${errorMessage}`],
					true
				);
			}
		} else {
	...
	function updateDataAndUIOnLanguageChange() {
		loadAllData().then(() => {
			if (!information || !projects || !skills || !contact) return;
			...
			portfolioData.directories = {
				...
				'~/projects/relevant': projects.relevant.projects.map((project: { name: string }) =>
					sanitizeFileName(project.name)
				),
				'~/projects/personal': projects.personal.projects.map((project: { name: string }) =>
					sanitizeFileName(project.name)
				),
				'~/projects/professional': projects.professional.projects.map((project: { name: string }) =>
					sanitizeFileName(project.name)
				),
			...
			projects.relevant.projects.forEach(
			...
			projects.personal.projects.forEach(
				(project: { name: string; description: string; url: string }) => {
			...
			projects.professional.projects.forEach(
				(project: { name: string; description: string; url: string }) => {
			...
			skills.skills.forEach((skill: { name: string; items?: string[] }) => {
