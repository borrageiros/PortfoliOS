<script lang="ts">
	...
	function resolveProfileTab(tab: string | null): string {
		if (tab && validProfileTabs.has(tab)) return tab;
		return 'information';
	}
	...
	const desktopIcons = [
		...
		},
		{
			...
			icon: 'gamepad',
			color: '#e74c3c',
			onClick: () => createNewWindow(gamesWindow)
		},
		{
	...
	$: sessionProfile = $page.url.searchParams.get('session') || '';
	$: desktopBackground = validProfiles.has(sessionProfile)
		? `/images/backgrounds/${sessionProfile}.jpg`
		: '/images/backgrounds/general.jpg';
	$: profileTabFromQuery = resolveProfileTab($page.url.searchParams.get('profile'));
	$: if (windows[0] && windows[0].content === (Profile as typeof SvelteComponent)) {
		windows[0].props = { initialTab: profileTabFromQuery };
	}

	// Variables to manage the state of the windows
	let nextId = 8;
	let _activeWindowIndex = 0;

	// Array to store window component references
	let windowRefs: Array<{ restore?: () => void }> = [];
	...
	function closeWindow(index: number): void {
		const closingWindow = windows[index];
		if (closingWindow?.content === (Profile as typeof SvelteComponent)) {
			const params = new URLSearchParams($page.url.searchParams);
			params.delete('profile');
			const queryString = params.toString();
			const nextUrl = queryString ? `${$page.url.pathname}?${queryString}` : $page.url.pathname;
	...
	function createNewWindow(window: WindowData, tab?: string): void {
		const maxZIndex = windows.length > 0 ? Math.max(...windows.map((win) => win.zIndex)) : 0;
		...
		nextId++;
		_activeWindowIndex = windows.length - 1;
	}
	...
	function handleDesktopMouseDown(event: MouseEvent) {
		...
		if (
			(event.button === 0 && (event.target as HTMLElement).classList.contains('desktop')) ||
		...
		<svelte:fragment slot="taskbar-items">
			{#each windows as window, index (window.id)}
				<div
					...
					on:click={() =>
						window.isMinimized ? restoreWindowFromTaskbar(index) : minimizeWindow(index)}
					on:keydown={handleKeydown}
					role="button"
					tabindex="0"
					title={window.title}
